...
/**
 * @file
 * Contains \Greencko\composer\ScriptHandler.
 */
...
class ScriptHandler {
  ...
  /**
   * Set the Drupal root directory, if at special environments like ispconfig
   *
  ...
  public static function createRequiredFiles(Event $event) {
    ...
    if (!$fs->exists($drupal_root . '/sites/default/settings.production.php') and $fs->exists($drupal_root . '/profiles/greencko/src/env/settings.production.php')) {
      $fs->copy($drupal_root . '/profiles/greencko/src/env/settings.production.php', $drupal_root . '/sites/default/settings.production.php');

      $fs->chmod($drupal_root . '/sites/default/settings.production.php', 0666);
      $event->getIO()
        ->write("Create a sites/default/settings.production.php file with chmod 0666");
    }
    if (!$fs->exists($drupal_root . '/sites/default/settings.staging.php') and $fs->exists($drupal_root . '/profiles/greencko/src/env/settings.staging.php')) {
      $fs->copy($drupal_root . '/profiles/greencko/src/env/settings.staging.php', $drupal_root . '/sites/default/settings.staging.php');
      $fs->chmod($drupal_root . '/sites/default/settings.staging.php', 0666);
      $event->getIO()
        ->write("Create a sites/default/settings.staging.php file with chmod 0666");
    }
    if (!$fs->exists($drupal_root . '/sites/default/settings.development.php') and $fs->exists($drupal_root . '/web/profiles/greencko/src/env/settings.development.php')) {
      $fs->copy($drupal_root . '/profiles/greencko/src/env/settings.development.php', $drupal_root . '/sites/default/settings.development.php');
      require_once $drupal_root . '/core/includes/bootstrap.inc';
      require_once $drupal_root . '/core/includes/install.inc';
      $settings['config_directories'] = [
        CONFIG_SYNC_DIRECTORY => (object) [
          'value' => Path::makeRelative($drupalFinder->getVendorDir() . '../config/sync', $drupal_root),
          'required' => TRUE,
        ],
      ];
      drupal_rewrite_settings($settings, $drupal_root . '/sites/default/settings.development.php');
      $fs->chmod($drupal_root . '/sites/default/settings.development.php', 0666);
      $event->getIO()
        ->write("Create a sites/default/settings.php file with chmod 0666");
    }
    ...
    if ($fs->exists($drupal_root . '/profiles/greencko/src/env/development.services.yml')) {
      $fs->copy($drupal_root . '/profiles/greencko/src/env/development.services.yml', $drupal_root . '/sites/development.services.yml');
      $fs->chmod($drupal_root . '/sites/development.services.yml', 0666);
      $event->getIO()
        ->write("Create a sites/development.services.yml file with chmod 0666");
    }
  ...
  public static function postDrupalScaffoldProcedure(Event $event) {
    ...
    if ($fs->exists($drupal_root . '/profiles/greencko/src/env/development.services.yml')) {
      // Alter development.services.yml to have Greencko's Local development
      // services.
   ...
   *
   *  Remove the distribution item for the parent greencko profile, If we want
   * to
   *  use the sub proifle as the distribution cover on the install step.
   *
   *  Can be used in the composer.json file of a Sub Profile of greencko
   *
   *  For Example:
   *  -------------------------------------------------------------------------
